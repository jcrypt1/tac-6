...
async def export_table(table_name: str):
    ...
    try:
        ...
        # Get all data from the table using safe query execution
...
async def export_results(request: ExportResultsRequest):
    ...
    try:
        ...
        for row in request.results:
            row_values = [row.get(col, '') for col in request.columns]
            writer.writerow(row_values)

        csv_content = output.getvalue()
        output.close()

        logger.info(f"[SUCCESS] Results exported: columns={len(request.columns)}, rows={len(request.results)}")
